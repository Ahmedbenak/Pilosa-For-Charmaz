...
# ── Personnalisation Charmaz ──────────────────────────────────────────────────
charmaz_corpus_path: entretiens/
charmaz_corpus_formats:
  - .txt
  - .md
  - .docx

# Chemins stables (cumulatifs entre les lancements — NE PAS déplacer)
charmaz_tracabilite_path: methodologie/tracabilite.xlsx
charmaz_checkpoint_path: methodologie/checkpoint.json
charmaz_etat_avancement_path: methodologie/ETAT_AVANCEMENT.md
charmaz_methodologie_path: methodologie/methodologie.md
charmaz_memos_path: methodologie/memos/
charmaz_script_tracabilite: methodologie/gerer_tracabilite.py

# Dossiers de session (logs uniquement — pointent vers les fichiers stables)
charmaz_runs_path: methodologie/run_<AAAAMMJJ_HHMM>/

# Comportement au lancement
charmaz_statut_par_defaut: interrompu
charmaz_resume_automatique: true
```
